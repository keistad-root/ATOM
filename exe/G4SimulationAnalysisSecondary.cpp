...
CppConfigFile setEnvironment(const ArgumentParser& parser) {
	...
	io::CSVReader<3> infoCSV(INFORMATION_PATH);
	infoCSV.read_header(io::ignore_extra_column, "TAG", "SIMULATION_FILE", "PLOT_FILE");
	std::string tags, inputFile, outputFile;
	while ( infoCSV.read_row(tags, inputFile, outputFile) ) {
