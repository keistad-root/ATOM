...
int main(int argc, char** argv) {
    ...
    // Analysis Manager
    AnalysisManager* analysisManager = new AnalysisManager(config.getConfig("Analysis").value().find("output_file"));
    
    // Geometry construction
    DetectorConstruction* detectorConstructor = new DetectorConstruction();
    DetectorConstruct(detectorConstructor, config.getConfig("Geometry").value());
    ...
    SetParticleSource(particleGun, config.getConfig("Source").value());

    runManager->Initialize();

    std::ifstream distFile(config.getConfig("Energy").value().find("alpha_energy_distribution"));
    std::string line;
    ...
    while(getline(distFile, line)) {
        ...
        double energy = Quantity(energyStr + "keV").getNum("MeV");
        int intensity = static_cast<int>(round(stod(intensityStr)*20000));
        particleGun->GetEneDist()->SetMonoEnergy(energy * MeV); // /gps/pos/mono 5.4 MeV
