...
TExperimentData::TExperimentData(const TExperimentData& copy) : mEvents(copy.mEvents), mClusters(copy.mClusters) {
	std::cout << "mEvets" << mEvents.size() << std::endl;
	std::cout << "mClusters" << mClusters.size() << std::endl;

}

TExperimentData& TExperimentData::operator=(const TExperimentData& copy) {
	mEvents.clear();
	mEvents.assign(copy.mEvents.begin(), copy.mEvents.end());

	mClusters.clear();
	mClusters.assign(copy.mClusters.begin(), copy.mClusters.end());
