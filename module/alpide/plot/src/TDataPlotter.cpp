...
void TDataPlotter::savePlots() {
	...
	if ( isClustermapProjectionX ) {
		...
		TF1* fitFunc = new TF1("fitFunc", "[0]/((x-[1])^2+[2]^2)+[3]*e^(-((x-[4])/[5])^2)+[6]", 1, ALPIDECOLUMN - 1);
		// fitFunc->SetParameter(0, mClustermapProjectionX->GetMaximum());
		fitFunc->SetParLimits(0, 1, mClustermapProjectionX->GetMaximum() * 100);
		fitFunc->SetParLimits(1, mClustermapProjectionX->GetMean() - 100, mClustermapProjectionX->GetMean() + 100);
		fitFunc->SetParameter(2, mClustermapProjectionX->GetStdDev());
		fitFunc->SetParameter(3, mClustermapProjectionX->GetMaximum() - mClustermapProjectionX->GetMinimum());
		fitFunc->SetParameter(4, mClustermapProjectionX->GetMean());
		fitFunc->SetParameter(5, mClustermapProjectionX->GetStdDev());
		fitFunc->SetParameter(6, mClustermapProjectionX->GetMinimum());
		mClustermapProjectionX->Fit(fitFunc, "R");
		TF1* lorents = new TF1("lorents", "[0]/((x-[1])^2+[2]^2)", 1, ALPIDECOLUMN - 1);
		lorents->SetParameters(fitFunc->GetParameter(0), fitFunc->GetParameter(1), fitFunc->GetParameter(2));
		lorents->SetLineColor(kBlue);
		lorents->SetLineWidth(4);
		TF1* gaus = new TF1("gaus", "[0]*e^(-((x-[1])/[2])^2)", 1, ALPIDECOLUMN - 1);
		gaus->SetParameters(fitFunc->GetParameter(3), fitFunc->GetParameter(4), fitFunc->GetParameter(5));
		gaus->SetLineColor(kMagenta);
		gaus->SetLineWidth(4);
		TF1* constant = new TF1("constant", "[0]", 1, ALPIDECOLUMN - 1);
		constant->SetParameter(0, fitFunc->GetParameter(6));
		constant->SetLineColor(kGreen + 3);
		constant->SetLineWidth(4);

		// TText* text = new TText(0.5, 0.5, Form("Mean: %.0f", fitFunc->GetParameter(2)));
		// text->SetNDC();
		// text->SetTextAlign(22);
		// text->Draw("SAME");
	...
	if ( isClustermapProjectionY ) {
		...
		TF1* fitFunc = new TF1("fitFunc", "[0]/((x-[1])^2+[2]^2)+[3]*e^(-((x-[4])/[5])^2)+[6]", 1, ALPIDECOLUMN - 1);
		fitFunc->SetParameter(0, mClustermapProjectionY->GetMaximum());
		fitFunc->SetParameter(1, mClustermapProjectionY->GetMean());
		fitFunc->SetParameter(2, mClustermapProjectionY->GetStdDev());
		fitFunc->SetParameter(3, mClustermapProjectionY->GetMaximum() - mClustermapProjectionY->GetMinimum());
		fitFunc->SetParameter(4, mClustermapProjectionY->GetMean());
		fitFunc->SetParameter(5, mClustermapProjectionY->GetStdDev());
		fitFunc->SetParameter(6, mClustermapProjectionY->GetMinimum());
