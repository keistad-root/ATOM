...
void TDecoder::DecodeAptsEvent() {
    uint8_t* temp_data = new uint8_t[len_event_data_];
    ...
    for (Int_t frame = 0; frame < nframes; frame++) {
        ...
        for(Int_t j = 0; j < 16; j++) {
            Int_t temp1d[8] = {0,};
            Int_t* temp1 = to_binary(temp1d, temp_data[fframe + 2 * j]);
            std::copy(temp1,temp1+8,std::begin(temp[j]));
            Int_t* temp2 = to_binary(temp1d, temp_data[fframe + 2 * j + 1]);
...
Int_t* TDecoder::to_binary(Int_t temp1d[], Int_t num1) {
    Int_t share1 = num1;
    Int_t rest1 = 0;
