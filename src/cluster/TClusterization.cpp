...
void TClusterization::addNewCluster(const std::array<int, 2>& pixel, std::vector<TCluster*>& clusters, int iEvent, int iTime) {
    TCluster* cluster = new TCluster(iEvent, iTime);
    cluster->AddPixel(pixel);
    clusters.push_back(cluster);
}

void TClusterization::removeIndependentCluster(const std::array<int, 2>& pixel, std::vector<TCluster*>& clusters) {
    if (clusters[0]->getMaxY() + 2 < pixel[1]) {
        int cut = 0;
...
bool TClusterization::clusterLitmusTest(const std::array<int, 2>& pixel, std::vector<TCluster*>& clusters) {
